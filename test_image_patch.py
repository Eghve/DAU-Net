...
import os
import numpy as np
import datasets.fish_count as fish_count
...
parser.add_argument('--data-path', type=str,
                    help='dataset path')
parser.add_argument('--dataset', type=str, default='dgcd',
                    help='dataset name')
...
def test(args, isSave = True):
    ...
    if args.dataset.lower() == 'dgcd':
        dataset = fish_count.Fish(os.path.join(data_path, 'test_data'), crop_size, 8, method='val')
    else:
    ...
    rmse = np.sqrt(np.mean(np.square(image_errs)))
    mae = np.mean(np.abs(image_errs))
    print('{}: mae {}, rmse {}\n'.format(model_path, mae, rmse))
