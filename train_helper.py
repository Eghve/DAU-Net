...
from datasets.fish_count import Fish
...
class Trainer(object):
    ...
    def setup(self):
        ...
        downsample_ratio = 8
        if args.dataset.lower() == "dgcd" or args.dataset.lower() == "ccd" or args.dataset.lower() == "ugcd":
            self.datasets = {
                "train": Fish(
        ...
        self.rmse = nn.MSELoss().to(self.device)
    ...
    def train_epoch(self):
        ...
        self.logger.info(
            ...
            "Count Loss: {:.2f}, TV Loss: {:.2f}, RMSE: {:.2f} MAE: {:.2f}, Cost {:.1f} sec".format(
                self.epoch,
                epoch_loss.get_avg(),
    ...
    def val_epoch(self):
        ...
        self.logger.info(
            "Epoch {} Val, RMSE: {:.2f} MAE: {:.2f}, Cost {:.1f} sec".format(
                self.epoch, rmse, mae, time.time() - epoch_start
            )
        )

        # log wandb
        wandb.log({"val/RMSE": rmse, "val/MAE": mae}, step=self.epoch)

        model_state_dic = self.model.state_dict()
        # if (2.0 * rmse + mae) < (2.0 * self.best_rmse + self.best_mae):
        ...
        if mae < self.best_mae:
            ...
            self.best_mae = mae
            self.logger.info(
                "save best rmse {:.2f} mae {:.2f} model epoch {}".format(
                    self.best_rmse, self.best_mae, self.epoch
                )
            )
